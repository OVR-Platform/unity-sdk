...
namespace BlueGraph
{
    ...
    public class Port : ISerializationCallbackReceiver
    {
        ...
        public virtual T GetValue<T>(T defaultValue = default)
        {
            ...
            {
                // ADDED BY OVER
                if (value.GetType() == typeof(Vector2) && typeof(T) == typeof(Vector3))
                {
                    Vector2 v = (Vector2)value;
                    object val = new Vector3(v.x, v.y, 0);
                    return (T)val;
                }

                if (value.GetType() == typeof(Vector3) && typeof(T) == typeof(Vector2))
                {
                    Vector3 v = (Vector3)value;
                    object val = new Vector2(v.x, v.y);
                    return (T)val;
                }
                // END
