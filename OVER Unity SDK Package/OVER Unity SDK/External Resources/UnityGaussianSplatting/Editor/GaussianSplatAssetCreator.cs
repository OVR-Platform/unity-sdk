...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        static string m_InputFile;
        static bool m_ImportCameras = true;

        static string m_OutputFolder = "Assets/GaussianAssets";
        static DataQuality m_Quality = DataQuality.Medium;
        static GaussianSplatAsset.VectorFormat m_FormatPos;
        static GaussianSplatAsset.VectorFormat m_FormatScale;
        static GaussianSplatAsset.ColorFormat m_FormatColor;
        static GaussianSplatAsset.SHFormat m_FormatSH;

        static string m_ErrorMessage;
        static string m_PrevPlyPath;
        static int m_PrevVertexCount;
        static long m_PrevFileSize;

        static bool isUsingChunks =>
        ...
        #region Over
        public static bool AutomateCreation(string inputPath, string outputPath, DataQuality dataQuality)
        {
            m_InputFile = inputPath;
            m_ImportCameras = true;
            ...
            string newOutputFolder = outputPath;
            ...
            m_FormatPos = GaussianSplatAsset.VectorFormat.Norm11;// (GaussianSplatAsset.VectorFormat)EditorGUILayout.EnumPopup("Position", m_FormatPos);
            m_FormatScale = GaussianSplatAsset.VectorFormat.Norm11;//(GaussianSplatAsset.VectorFormat)EditorGUILayout.EnumPopup("Scale", m_FormatScale);
            m_FormatColor = GaussianSplatAsset.ColorFormat.Norm8x4; // (GaussianSplatAsset.ColorFormat)EditorGUILayout.EnumPopup("Color", m_FormatColor);
            m_FormatSH = GaussianSplatAsset.SHFormat.Norm6;// (GaussianSplatAsset.SHFormat)EditorGUILayout.EnumPopup("SH", m_FormatSH);

            if (m_FormatSH >= GaussianSplatAsset.SHFormat.Cluster64k)
            {
                Debug.LogWarning($"Warning! SH clustering is not fast! (3-10 minutes for 6M splats)");
            }

            //if (totalSize > 0)
            //    Debug.Log($"Asset Size: {EditorUtility.FormatBytes(totalSize)} - {(double)m_PrevFileSize / totalSize:F2}x smaller");
        ...
        unsafe static void CreateAsset()
        {
            m_ErrorMessage = null;
        ...
        unsafe static NativeArray<InputSplatData> LoadPLYSplatFile(string plyPath)
        {
        ...
        static void CreatePositionsData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
        ...
        static void CreateOtherData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash, NativeArray<int> splatSHIndices)
        {
        ...
        static void CreateColorData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
        ...
        static void CreateSHData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash, NativeArray<GaussianSplatAsset.SHTableItemFloat16> clusteredSHs)
        {
