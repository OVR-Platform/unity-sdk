...
namespace BlueGraph.Editor
{
    ...
    internal class SearchGroup
    {
        ...
        internal void AddToTree(List<SearchTreeEntry> tree)
        {
            ...
            {
                /// ADDED BY OVER
                string prefix = string.IsNullOrEmpty(result.Prefix) ? "" : result.Prefix + " ";
                GUIContent content = new GUIContent("      " + prefix + result.Name);

                if(!string.IsNullOrEmpty(result.Icon))
                {
                    string iconPath = Path.Combine("Packages/com.over.over-unity-sdk", "Editor Default Resources", $"Visual Scripting Icons/{result.Icon}.png");
                    Texture icon = EditorGUIUtility.Load(iconPath) as Texture;
                    
                    content = new GUIContent(" "+ prefix + result.Name, icon);
                }
